            ...
            else
            {
                ...
                if (Server_CheckGameOver())
                {
                    if (g_CurrentStage == MAX_STAGE) {
                        printf("[Game] 모든 스테이지 클리어! 게임 종료!\n");

                        // 게임 최종 점수, 등수 계산
                        Server_EndMatch();

                        // "게임 종료" 패킷 전송 (S2C_GameOverPacket)
                        S2C_GameOverPacket overPkt;

                        for (int i = 0; i < MAX_PLAYERS; i++) {
                            overPkt.playerEnd[i] = g_endScores[i];
                        }

                        for (int i = 0; i < MAX_PLAYERS; i++) {
                            if (g_clients[i].IsConnected) {
                                send(g_clients[i].TCPSocket, (char*)&overPkt, sizeof(overPkt), 0);
                            }
                        }

                        g_IsGameRunning = false;
                        printf("[Game] 마지막 스테이지 클리어, 게임 종료.\n");
                        break;
                    }
                    else {
                        // 대기 모드 진입
                        ...
                        // for (int i = 0; i < MAX_PLAYERS; i++) {
                            // playerStage[i] = g_stageScores[i][g_CurrentStage - 1];
                        // }
