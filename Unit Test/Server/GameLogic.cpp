...
StageScore g_stageScores[MAX_PLAYERS][3] = {};
...
void Server_EndMatch() {
    for (int i = 0; i < MAX_PLAYERS; ++i) {
        PlayerGameStats& stats = g_clients[i].playerStats;
        // 총점 계산 (주차 시간 1초당 -5점, 충돌 1회당 -10점), 만점 = 각 STAGE별로 1000점
        if (1000 - (int)(stats.ParkingSec * 5.0f) - (stats.CollisionCount * 10) < 0) g_stageScores[i][g_CurrentStage - 1].score = 0;
        else {
            g_stageScores[i][g_CurrentStage - 1].score = 1000 - (int)(stats.ParkingSec * 5) - (stats.CollisionCount * 10);
        }
        printf("[서버] 플레이어 %d 스테이지 %d 점수: %d \n", i, g_CurrentStage, g_stageScores[i][g_CurrentStage - 1].score);
    }
    ...
    for (int i = 0; i < MAX_PLAYERS; ++i) {
        ...
        for (int j = 0; j < MAX_PLAYERS; ++j) {
            if (i == j) continue;
            if (g_stageScores[j][g_CurrentStage - 1].score > g_stageScores[i][g_CurrentStage - 1].score) {
                rank++;
            }
        }

        g_stageScores[i][g_CurrentStage - 1].rank = rank;
        printf("[서버] 플레이어 %d 스테이지 %d 등수: %d \n", i, g_CurrentStage, g_stageScores[i][g_CurrentStage - 1].rank);
    }

    if (g_CurrentStage == MAX_STAGE) {
        // 최종 점수 계산, 각 stage별 가중치를 둬서 점수 합산, 만점 = 5000점
		for (int i = 0; i < MAX_PLAYERS; ++i) {
			g_endScores[i].Finalscore = g_stageScores[i][0].score * 1 + g_stageScores[i][1].score * 2 + g_stageScores[i][2].score * 3;
			printf("[서버] 플레이어 %d 최종 점수: %d \n", i, g_endScores[i].Finalscore);
		}
		// 등수 계산
        for (int i = 0; i < MAX_PLAYERS; ++i) {
            int rank = 1;
            for (int j = 0; j < MAX_PLAYERS; ++j) {
                if (i == j) continue;
                if (g_endScores[j].Finalscore > g_endScores[i].Finalscore) {
                    rank++;
                }
            }
            g_endScores[i].rank = rank;
            printf("[서버] 플레이어 %d 최종 등수: %d \n", i, g_endScores[i].rank);
        }
    }
}