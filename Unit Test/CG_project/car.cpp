...
static int MyIdx() {
	...
	if (PlayerId >= kCarCount) return 0;
	return PlayerId;
}

std::vector<std::pair<float, float>> Car_GetRotatedCorners()
{
	return Car_GetRotatedCorners(MyIdx());
}
...
float Car_GetDX() { return Car_GetDX(MyIdx()); }
float Car_GetDY() { return Car_GetDY(MyIdx()); }
float Car_GetDZ() { return Car_GetDZ(MyIdx()); }
float Car_GetRotationY() { return Car_GetRotationY(MyIdx()); }
float Car_GetWheelRotationX() { return Car_GetWheelRotationX(MyIdx()); }
float Car_GetSpeed() { return Car_GetSpeed(MyIdx()); }
