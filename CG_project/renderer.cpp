...
static void drawFinishRect(int modelLoc)
{
	...
	// 주차장 모델 바인딩 (한 번만 하면 됨)
	glBindVertexArray(vao[5]);

	// ★ 4개의 주차장을 순서대로 그리기 (i: 0 ~ 3) ★
	for (int i = 0; i < 4; i++)
	{
		// 1. 위치 설정: "i번째 주차장 위치로 가라!"
		// (아까 Environment 쪽 함수에 인자(i) 추가한 게 여기서 쓰입니다)
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(Environment_GetFinishRectMatrix(i)));

		// 2. 색상 설정: "i번째 주차장은 무슨 색?"
		if (GameState_IsParked())
		{
			// 게임 클리어(주차 성공) 시 전부 녹색으로 변경 (축하 효과)
		...
		// 4. 바닥 그리기 (회색 부분)
		// 바닥은 항상 회색이어야 하므로 색을 다시 회색으로 바꿈
		glUniform3f(objColorLocation, 0.5f, 0.5f, 0.5f);
...
{
	// ---------------------------------------------------------
	// 1. 장식용 주차공간 바닥 (흰색 라인 + 회색 바닥)
	// ---------------------------------------------------------
	glBindVertexArray(vao[8]); // 주차장 모델 바인딩
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 5; j++)
		{
			// 아까 수정한 GetObstacleMatrix(i, j) 호출 (인자 2개!)
	...
	if (currentStage == 3)
	{
		...
		for (int j = 0; j < 2; j++)
		{
			glm::mat4 obsMat = Environment_GetObstacleMatrix(0, j); // 0번 구역의 0, 1번 장애물

			// 길게 늘리기 (스케일링)
			obsMat = glm::scale(obsMat, glm::vec3(3.0f, 2.0f, 8.0f));

			glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(obsMat));
			glDrawArrays(GL_TRIANGLES, 0, TRI_COUNT * 3);
		}
	}
	// ★ [Stage 1, 2] 작은 박스 5개씩 그리기 (여기가 1라운드용!) ★
	else
	{
		glUniform3f(objColorLocation, 0.25f, 0.25f, 0.25f); // 회색

		// 4개 구역 * 5개 장애물 = 총 20개 반복
		for (int i = 0; i < 4; i++)
		{
			for (int j = 0; j < 5; j++) // ★ 여기서 5개를 그립니다 ★
			{
				glm::mat4 obsMat = Environment_GetObstacleMatrix(i, j);
				glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(obsMat));
				glDrawArrays(GL_TRIANGLES, 0, TRI_COUNT * 3);
