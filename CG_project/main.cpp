...
DWORD WINAPI Client_TCP(LPVOID lpParam) {
    ...
    serverAddr.sin_port = htons(9000); // 서버 포트 번호 (서버와 맞춰야 함)
    serverAddr.sin_addr.s_addr = inet_addr("127.0.0.1"); // 서버 IP (로컬 테스트용)
    ...
    while (isRunning)
    {
        // --- 데이터 보내기 (Send) ---
        {
            // [자물쇠 잠금] 메인 스레드가 데이터를 수정하는 동안은 기다림
            std::lock_guard<std::mutex> lock(dataMutex);

            // 구조체 통째로 전송
            int sendResult = send(serverSocket, (char*)&myData, sizeof(myData), 0);

            if (sendResult == SOCKET_ERROR) {
                std::cout << "서버 연결 끊김\n";
                break;
            }
        } // [자물쇠 해제] 여기서 lock_guard가 사라지며 자동으로 잠금 해제

        // --- (선택) 서버에서 데이터 받기 (Recv) ---
        // char buffer[1024];
        // recv(serverSocket, buffer, ...);

        // 너무 자주 보내지 않도록 약간의 딜레이 (예: 33ms = 약 30FPS)
