...
unsigned int WINAPI TCP_HandleClient(LPVOID arg) {
	...
	while (true) {
        ...
        if (pkt->myData.W_Pressed) g_clients[playerID].LastReceivedKeys.W_Pressed = true;
        if (pkt->myData.SPACE_Pressed) g_clients[playerID].LastReceivedKeys.SPACE_Pressed = true;
        if (pkt->myData.Q_Pressed) g_clients[playerID].LastReceivedKeys.Q_Pressed = true;
        if (pkt->myData.E_Pressed) g_clients[playerID].LastReceivedKeys.E_Pressed = true;

        if (pkt->myData.P_Pressed) {
            /* 정해진 주차 구역 안에 들어가서 p누른 경우 -> 주차 영역에 들어갔는지 확인하는 함수 구현 전이라 주석처리
            if () {
                g_clients[playerID].LastReceivedKeys.W_Pressed = true;
                // 주차 완료 표시로 바꾸기
                EnterCriticalSection(&g_cs);

                g_clients[playerID].playerStats.IsParked = true;
                g_clients[playerID].playerStats.ParkingSec = (float)(GetTickCount() - g_GameStartTime) / 1000.0f;
        ...
        if (pkt->myData.handle_rotate_z) g_clients[playerID].LastReceivedKeys.handle_rotate_z = pkt->myData.handle_rotate_z;
	}
